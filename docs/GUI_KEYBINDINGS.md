...
| Disable Ghostty forwarding | Comment the `config-file = ~/<your-dotfiles>/misc/ghostty_gui_keybindings` line in `ghostty_config`, then reload Ghostty |
...
2. Restart Neovim.
3. Comment
   `config-file = ~/<your-dotfiles>/misc/ghostty_gui_keybindings`
   in `ghostty_config`.
